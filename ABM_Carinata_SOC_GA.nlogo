...
to profit
  ...
  let YC1 random-normal 129.91 15.93
  let PC1 random-poisson 4.90
  let CC1 random-normal 274.09 27.98
 ...
 ; file-close
  ...
  let YC2a random-normal 859.00 105.84
  let PC2a random-normal  0.75 0.09
  let YC2b random-normal 1389.55 171.19
  let PC2b random-normal 0.08 0.02
  let CC2 random-normal 572.43 35.72
...
;  set ProfC3 (YC3 *  PC3 - CC3)
 ...
 ; file-open "C:/ABMUpscale/Result/PenutProf.txt"
  let YC4 random-normal 3802 403.21
  let PC4 random-normal 0.26 0.05
  let CC4 random-normal 551.96 29.59
...
;;; For the begining of every ifelse profit calculation, the intial profit estiamtion at t=0 is the profit data from 2015-1017 with the Production Price Index of reference year 2019.
  ;;; For simplicity direct valuew were used without setting variables.
    ...
    if color = green and tSOC != "NaN" and tYield != "NaN" [
      ifelse t = 0
    [set ProfitT NP ^ (3 * t + 1) * 3 * (120.77 + 154.77 + 168.84)
        set ProfitB NP ^ (3 * t) * 120.77 + NP ^ (3 * t + 1) * 154.77 + NP ^ (3 * t + 1) * (CarinataPrice * tYield - CarinataCost + tSOC * SOCIncentive) + NP ^ (3 * t + 2) * 0.9 * 525.1]
    ...
    if color = yellow and tSOC != "NaN" and tYield != "NaN" [
      ifelse t = 0
      [set ProfitT NP ^ (3 * t) * 120.77 + NP ^ (3 * t + 1) * 154.77 + NP ^ (3 * t + 2) * 525.1
        set ProfitB NP ^ (3 * t) * 120.77 + NP ^ (3 * t + 1) * 154.77 + NP ^ (3 * t + 1) * (CarinataPrice * tYield - CarinataCost + tSOC * SOCIncentive) + NP ^ (3 * t + 2) * 0.9 * 525.1]
    ...
    if color = white and tSOC != "NaN" and tYield != "NaN" [
      ifelse t = 0
     [set ProfitT NP ^ (3 * t) * 120.77 + NP ^ (3 * t + 1) * 154.77 + NP ^ (3 * t + 2) * 129.38
        set ProfitB NP ^ (3 * t) * 120.77 + NP ^ (3 * t + 1) * 154.77 + NP ^ (3 * t + 1) * (CarinataPrice * tYield - CarinataCost + tSOC * SOCIncentive) + NP ^ (3 * t + 2) * 0.9 * 525.1]
