...
to setup
  ...
  gis:apply-coverage GA-dataset "SOCNT" SOC       ; SOCB - Base Scenario; SOCNT - No-till scenario
  gis:apply-coverage GA-dataset "YIELDNT" Yield   ;YIELDB - Base Scenario; YIELDNT - No-till scenario
...
to profit
  ...
  let YC1 random-normal 129.50 17.56
  let PC1 random-poisson 4.63
  let CC1 random-normal 427.0 60.94
 ...
 ; file-close
  ...
  let YC2a random-normal 841.30 99.31
  let PC2a random-normal  0.75 0.11
  let YC2b random-normal 1361.00 160.81
  let PC2b random-normal 0.08 0.02
  let CC2 random-normal 612.78 43.68
...
;  set ProfC3 (YC3 *  PC3 - CC3)
 ...
 ; file-open "C:/ABMUpscale/Result/PenutProf.txt"
  let YC4 random-normal 4095.10 366.19
  let PC4 random-normal 0.21 .04
  let CC4 random-normal 586.95 47.00
...
to cultivate
    ...
    let CarinataCost 286.32
  ;;Profit of crops for 2015-2017
    let c1_15 146.2409644
    let c1_16 76.57567157
    let c1_17 198.4979267

    let c2_15 62.26866018
    let c2_16 89.37393296
    let c2_17 107.8777165

    let c4_15 244.0935401
    let c4_16 128.7079537
    let c4_17 412.2473469




    ;file-open "Output.txt"  ;; Opening file for writing
;;; For the begining of every ifelse profit calculation, the intial profit estiamtion at t=0 is the profit data from 2015-1017 with the Production Price Index of reference year 2019.
  ;;; For simplicity direct valuew were used without setting variables.

    ask turtles [

    if color = grey and tSOC != "NaN" and tYield != "NaN" [
      ifelse t = 0
    [set ProfitT NP ^ (3 * t + 1) * 3 * (c2_15 + c2_16 + c2_17)
        set ProfitB NP ^ (3 * t) * c2_15 + NP ^ (3 * t + 1) * c2_16 + NP ^ (3 * t + 1) * (CarinataPrice * tYield - CarinataCost + tSOC * SOCIncentive) + NP ^ (3 * t + 2) * 0.9 * c4_17]
    ...
    if color = grey + 1 and tSOC != "NaN" and tYield != "NaN" [
      ifelse t = 0
      [set ProfitT NP ^ (3 * t) * c2_15 + NP ^ (3 * t + 1) * c2_16 + NP ^ (3 * t + 2) * c4_17
        set ProfitB NP ^ (3 * t) * c2_15 + NP ^ (3 * t + 1) * c2_16 + NP ^ (3 * t + 1) * (CarinataPrice * tYield - CarinataCost + tSOC * SOCIncentive) + NP ^ (3 * t + 2) * 0.9 * c2_17]
    [set ProfitT NP ^ (3 * t) * ProfC2 + NP ^ (3 * t + 1) * ProfC2 + NP ^ (3 * t + 2) * ProfC4
        set ProfitB NP ^ (3 * t) * ProfC2 + NP ^ (3 * t + 1) * ProfC2 + NP ^ (3 * t + 1) * (CarinataPrice * tYield - CarinataCost + tSOC * SOCIncentive) + NP ^ (3 * t + 2) * 0.9 * ProfC4]
    ]

    if color = grey + 2 and tSOC != "NaN" and tYield != "NaN" [
      ifelse t = 0
     [set ProfitT NP ^ (3 * t) * c2_15 + NP ^ (3 * t + 1) * c2_16 + NP ^ (3 * t + 2) * c1_17
        set ProfitB NP ^ (3 * t) * c2_15 + NP ^ (3 * t + 1) * c2_16 + NP ^ (3 * t + 1) * (CarinataPrice * tYield - CarinataCost + tSOC * SOCIncentive) + NP ^ (3 * t + 2) * 0.9 * c4_17]
   ...
   ifelse a? = true [set pcolor red] [set pcolor blue]        ;;we do not assume farmers having negetive experience will never adopt energy crop.
...
396
Cot3 Adopters (%)
precision (count turtles with [a? = TRUE and color = grey] / 3599 * 100) 2
17
1
11
...
396
Cpt2P1 Adopters (%)
precision (count turtles with [a? = TRUE and color = grey + 1] / 3599 * 100) 2
17
1
11
...
272
Cot3 Farmers (%)
precision (count turtles with [color = grey] / 3599 * 100) 2
17
1
11
...
273
Cot2P1 Famrers (%)
precision (count turtles with [color = grey + 1] / 3599 * 100) 2
17
1
11
...
273
Cot2Cr1 Farmers (%)
precision (count turtles with [color = grey + 2] / 3599 * 100) 2
17
1
11
...
395
Cot2Cr1 Adopters (%)
precision (count turtles with [a? = TRUE and color = grey + 2] / 3599 * 100) 2
17
1
11
